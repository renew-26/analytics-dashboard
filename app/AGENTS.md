...
## Purpose
Next.js App Router 루트 디렉토리. 전역 레이아웃(사이드바+헤더), 홈 대시보드, 렌탈사별 상세 페이지와 다수의 분석 페이지(브랜드·카테고리 트렌드·전환율·공헌이익·예외승인·렌탈사 비교·경쟁사 지원금), API 라우트를 포함한다. 대부분의 페이지는 `page.tsx`(Server Component, 데이터 fetch+집계) + `*Client.tsx`(Client Component, 인터랙션) 쌍으로 구성된다.
