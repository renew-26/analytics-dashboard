...
**토글은 mover 행의 `<Link>` 바깥에 둔다.** 위 스니펫은 배치가 아니라 구조를 보인
것이다 — `<a>` 안에 `<button>` 을 넣으면 유효하지 않은 HTML 이고 토글 클릭이 내비게이션을
발동시킨다. 같은 `<li>` 안, 행 바로 아래에 형제로 놓는다.

`expandable` 만으로는 TS 가 `sub: Mover[] | undefined` 를 좁히지 못한다 — JSX 조건에
`sub &&` 를 함께 쓴다.
...
### ⚠️ DW `dim_rental_company` 로 렌탈사 축을 대조하지 말 것

Task 6 이 렌탈사 축을 세우는데, **DW 의 렌탈사 차원은 앱의 렌탈사와 같은 축이 아니다.**
(2026-09-12, 렌탈사 화면 세션이 자기 사용자와 확인한 내용 — 이쪽 실측과도 부합한다)

- DW `dim_rental_company` 가 **브랜드를 렌탈사 축에 섞어 놓았다.** "삼성전자"가 렌탈사처럼
  잡히지만 실제로는 브랜드다.
- **이름 표기도 다르다** — DW `SK매직` = 앱 `SK인텔릭스`. 이쪽 실측에서도 앱은
  `rental_company=SK인텔릭스` / `brand=SK매직` 으로 두 축이 분리돼 있다.

그러므로 렌탈사별 수치는 DW 가 아니라 **운영 DB `PROP_ITEM.RENTAL_COMPANY`** 로 대조한다.
`lib/company-map.ts` 의 `COMPANY_MAP` 이 원천 표기를 화면 라벨로 접으므로, 대조할 때는
그 매핑을 통과시킨 뒤 비교해야 한다(예: 원천 `유버스(현대렌탈서비스)` → 라벨 `현대유버스`).
