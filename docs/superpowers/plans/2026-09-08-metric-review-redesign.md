...
> **날짜 이동 주의 (2026-09-09 확인):** Task 1 스냅샷은 기준일 2026-09-07(창 9/1~9/7)에 고정됐는데
> 페이지는 이미 기준일이 하루 밀려 9/1~9/8 을 집계한다. 렌더된 화면 숫자를 스냅샷과 직접 비교하면
> **하루치 차이가 전부 불일치로 뜬다.** 대사를 두 층으로 나눈다:
>
> ① **원천 안정성** — 스냅샷이 고정한 창(9/1~9/7)으로 Supabase 를 재질의해 스냅샷과 대조한다.
>    작업 중 원천이 움직이지 않았음을 확인하는 용도다. 아래 Step 1 스크립트가 이 층이다.
>
> ② **집계기 정합성** — 렌더된 페이지의 근거 줄(`데이터 기준 … · N행 (M/D~M/D)`)에서 **페이지가
>    스스로 밝힌 창과 행수**를 파싱하고, 그 창으로 Supabase 를 질의해 페이지에 찍힌 KPI 4값과
>    대조한다. 페이지가 자기 창을 적어 두므로 기준일이 며칠 더 밀려도 이 대사는 계속 성립한다.
>    **②가 이 개편의 진짜 게이트다** — 집계기·패널·페이지를 전부 거친 값을 검증한다.
>    두 화면(`/revenue-analysis`, `/transaction-count`) × 두 기준(`basis=order|contract`)에 대해 수행한다.
