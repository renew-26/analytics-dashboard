...
**외부 소비자 없음** — `raw_orders` · `raw_contracts` 를 이 저장소 밖에서 읽는 곳은
없다고 사용자가 확인했다(2026-09-12). 드롭을 막는 것이 없다.

**잔여 160건 판정 불필요** — 뷰 전환을 폐기하면서 이 질문도 함께 닫힌다.
그 160건은 구 `raw_orders` 에만 있고 4678 이 반환하지 않는 행이었는데, 구 테이블을
드롭하면 애초에 존재하지 않게 된다. 데이터레이크가 `raw_prop_items` 와 맞는다는
사용자 확인이 그 행들을 드리프트로 판정하는 근거다.
