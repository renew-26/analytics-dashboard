...
스택 정의는 `app/globals.css`의 `--font-pretendard` **한 곳**이고, 실제 로드는 `app/layout.tsx`의 `<link>`가 한다 — `globals.css`에 `@import` 를 쓰면 Turbopack이 산출물에서 제거한다. 스택에 이름만 적고 로드하지 않으면 보는 사람 로컬 설치 여부에 렌더가 갈린다.

### Scale

**사이즈는 정수만.** 소수점(12.5 / 13.5 같은)은 서브픽셀로 반올림돼 기기·줌 배율마다 다르게 앉는다.
line-height는 **4px 그리드**에 올린다 — 표 행 높이가 그리드에서 떨어지면 여러 표를 나란히 놓을 때 눈금이 안 맞는다.

| Role | Size / LH | Weight | Use |
|---|---|---|---|
| 마이크로 라벨 | 10 / 14 | 700 | 축 라벨, 태그 |
| 캡션 | 11 / 16 | 500 | 보조 설명 |
| 본문 · 표 셀 | 12 / 16 | 500 | 설명, 표 셀 (`text-xs`) |
| 강조 본문 · 패널 타이틀 | 14 / 20 | 600 | 카드·패널 헤더 (`text-sm`) |
| 섹션 헤딩 | 15 / 20 | 700 | 계층 구분 |
| 페이지 타이틀 | 18 / 24 | 700 | 상단바 h1 (`text-lg`) |
| 리드 문장 | 20 / 28 | 600 | 자동 생성 요약문 (`text-xl`) |
| KPI 값 | 24 / 28 | 700 | 히어로 타일 (`text-2xl`) |
| 카드 대표값 | 27 / 32 | 700 | 렌탈사 카드 |

**Weight는 700이 상한이다.** 800을 쓰지 않는다 — 가변축이 열려 있어서 800이 *가능*하지만, 12~15px 구간에서 800은 획이 서로 붙어 오히려 덜 또렷해진다. 또렷함은 굵기가 아니라 정수 사이즈 + 라인 그리드에서 나온다. 실사용은 500(본문) / 600(강조) / 700(타이틀) 세 단계로 끝낸다.

### 정리 대상 (스케일 밖 잔여, 총 39곳)
새로 쓰지 말고, 만지는 파일에서 보이면 목표값으로 흡수한다.

| 현재 | 목표 | 남은 곳 |
|---|---|---|
| 9px | 10px | 6 |
| 13px | 12px | 8 |
| 16px (`text-base`) | 15px | 20 |
| 19px | 18px | 2 |
| 21px | 20px | 1 |
| 22px | 24px | 2 |
...
- 같은 크기를 프리셋과 임의값으로 섞어 쓰지 않는다. 표에 프리셋이 적힌 단계는 프리셋을 쓴다.

### 미해결 — body 기본 line-height
현재 body에 `line-height`가 없어서 leading을 지정하지 않은 요소는 브라우저 기본값(≈1.2)으로 렌더된다. 한글 본문에는 좁고, TDS는 전역 1.45를 쓴다. **아직 적용하지 않았다** — 1.45로 올리면 leading 미지정 요소가 전부 늘어나 밀집 표의 행 높이가 함께 커진다. 시각 회귀를 확인할 수단이 생긴 뒤에 결정한다.
